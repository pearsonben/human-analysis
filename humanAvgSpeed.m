...
fprintf('Control Average Speed: %f\t', mean(gradients(1:end,1)));
fprintf('PD Average Speed: %f\n', mean(gradients(1:end,2)));
...
function gradients = plotData(dataControl, dataParkinsons, iterations)
    ...
    mean_ControlSpeed = mean(abs((control(TF3)-control(TF1))./((xRot(TF3)-xRot(TF1))*0.0142)));
    mean_ParkinsonsSpeed = mean(abs((parkinsons(TF4)-parkinsons(TF2))./((xRot(TF4)-xRot(TF2))*0.0142)));
    ...
    gradients = [mean_ControlSpeed mean_ParkinsonsSpeed];
